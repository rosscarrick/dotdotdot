...
local function nnoremap(desc, keys, command)
	vim.api.nvim_set_keymap("n", keys, command, { noremap = true, silent = true, desc = desc })
end

nnoremap("Save", "<Leader><Leader>", ":w<CR>")
nnoremap("Previous buffer", "<Leader>j", "<C-^>")
nnoremap("Toggle fold", "<tab>", "za")
nnoremap("Search/create tmux session from projects folder", "<Leader>P", ":!tmux neww ~/scripts/tmux-nav.sh<CR>")
